...
def _to_pixels_bytes(img: Image.Image, target_w: int, target_h: int) -> tuple[bytes, str]:
...
def generate_device_bin(request):
    ...
    try:
        ...
        source_prefix = body.get("source_prefix", f"characters/{character_id}/device/").strip("/") + "/"
        ...
        for base_name, img in sources:
            pixels, fmt = _to_pixels_bytes(img, width, height)
        ...
        manifest = {
            ...
            "version": version,
            "count": len(outputs),
            "base_path": f"{prefix}/{device_id}/{character_id}/",
            "bucket": bucket_name,
            "items": outputs,
        }
        manifest_rel = f"{prefix}/{device_id}/{character_id}/manifest.json"
