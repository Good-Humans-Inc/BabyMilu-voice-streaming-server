...
def _moment_ids(snapshot: Any) -> Tuple[str, str]:
    user = snapshot.reference.parent.parent
    data = snapshot.to_dict() or {}
    return (user.id if user is not None else "", str(data.get("characterId") or ""))


def _get_journal_memory_events(
    ...
    *,
    ...
    try:
        return sb.get_journal_memory_events(user_id, character_id=character_id, limit=limit)
    except TypeError:
        return sb.get_journal_memory_events(user_id, limit=limit)


def _journal_entries_from_memory_events(events: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
...
def _journal_display_at(local_date: str, timezone_name: str) -> str:
    tz = _zone(timezone_name)
    try:
        day = datetime.fromisoformat(local_date).date()
    except ValueError:
        day = datetime.now(tz).date()
    return datetime.combine(day, time(7, 0), tzinfo=tz).astimezone(timezone.utc).isoformat()
    ...
    *,
    ...
    for marker in waiting:
        ...
        try:
            ...
            turns = sb.get_turns(session_id)
            journal_events = _get_journal_memory_events(sb, user_id, character_id=character_id, limit=3)
            ...
            prior_journal_exists = bool(journal_events)
    ...
    *,
    ...
    for queue_doc in queues:
        ...
        try:
            ...
            journal_events = _get_journal_memory_events(sb, user_id, character_id=character_id, limit=10)
            prior_entries = _journal_entries_from_memory_events(journal_events)
            ...
            if execute:
                local_date = str(queue.get("date") or store.local_date_for_timezone(now, store.get_user_timezone(db, user_id)))
    ...
    for user_snap in db.collection("users").limit(config.max_generation_queues()).stream():
        ...
        try:
            ...
            for character_id in sb.get_character_ids_for_user(user_id, limit=20):
                ...
                existing_today = [entry for entry in prior if entry.get("displayDate") == local_date and entry.get("journalType") == "lure_back"]
                if existing_today:
                    continue
                if not execute:
...
def run_journal_publish_job(
    *,
    ...
    for entry_doc in entries:
        ...
        user_id, character_id = _moment_ids(entry_doc)
...
def _supersede_regular_if_needed(db: firestore.Client, user_id: str, character_id: str, entry: Dict[str, Any]) -> None:
    if entry.get("journalType") != "lure-back":
        return
    display_at = entry.get("displayAt")
    for candidate in store.list_journal_entries(db, user_id, character_id, include_deleted=True, limit=20):
        if candidate.get("displayAt") == display_at and candidate.get("journalType") == "regular" and candidate.get("status") == "ready":
            candidate["_ref"].set({"status": "superseded"}, merge=True)
...
def _send_journal_push(
    ...
    data = {
        ...
        "displayAt": str(entry.get("displayAt") or ""),
    }
    try:
